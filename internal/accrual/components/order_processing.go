package components

import (
	"context"
	"github.com/Erlast/loyalty-gophermart.git/pkg/zaplog"
	"strings"
	"time"

	"github.com/Erlast/loyalty-gophermart.git/internal/accrual/helpers"
	"github.com/Erlast/loyalty-gophermart.git/internal/accrual/storage"
)

var timeSleep = 1 * time.Minute
var percentFull int64 = 100

func OrderProcessing(ctx context.Context, store storage.Storage) {
	for {
		orders, err := store.GetRegisteredOrders(ctx)
		if err != nil {

			zaplog.Logger.Errorf("ошибка при попытке выбрать новые заказы: %v", err)
		}

		rules, err := store.FetchRewardRules(ctx)
		if err != nil {
			zaplog.Logger.Error("не могу выбрать правила начислений")
		}

		for _, orderID := range orders {
			err = store.UpdateOrderStatus(ctx, orderID, helpers.StatusProcessing)
			if err != nil {
				zaplog.Logger.Error("невозможно обновоить статус заказа")
			}

			products, err := store.FetchProducts(ctx, orderID)

			if err != nil {
				zaplog.Logger.Error("не могу получить товары из заказа", err)
				err = store.UpdateOrderStatus(ctx, orderID, helpers.StatusInvalid)
				if err != nil {
					zaplog.Logger.Error("невозможно обновоить статус заказа", err)
				}
			}

			points := make([]int64, len(products))

			for i, product := range products {
				for _, rule := range rules {
					if strings.Contains(product.Description, rule.Match) {
						switch rule.RewardType {
						case "%":
							points[i] += int64((product.Price * float64(rule.Reward)) / float64(percentFull))
						case "pt":
							points[i] += rule.Reward
						default:
							points[i] += 0
						}
					}
				}
			}

			err = store.SaveOrderPoints(ctx, orderID, points)
			if err != nil {
				zaplog.Logger.Error("не могу сохранить информацию о заказе. ", err)
				err = store.UpdateOrderStatus(ctx, orderID, helpers.StatusInvalid)
				if err != nil {
					zaplog.Logger.Error("невозможно обновоить статус заказа", err)
				}
			}
		}

		time.Sleep(timeSleep)
	}
}
